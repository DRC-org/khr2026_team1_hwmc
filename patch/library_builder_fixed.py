# https://github.com/micro-ROS/micro_ros_platformio/pull/189
...
set(__BIG_ENDIAN__ 0)"""
        ...
        cmake_toolchain = cmake_toolchain.format(
            C_COMPILER=cc,
            CXX_COMPILER=cxx,
            AR_COMPILER=ar,
            C_FLAGS=cflags,
...
class Build:
    ...
    def build_dev_environment(self):
        ...
        if self.distro in ("rolling", "kilted"):
            ...
            command = "cd {} && {} . {} && colcon build --packages-ignore rmw_test_fixture --cmake-args -DBUILD_TESTING=OFF -DPython3_EXECUTABLE=`which python`".format(
                self.dev_folder, touch_command, self.python_env
            )
        else:
            command = "cd {} && . {} && colcon build --cmake-args -DBUILD_TESTING=OFF -DPython3_EXECUTABLE=`which python`".format(
                self.dev_folder, self.python_env
            )
    ...
    def download_mcu_environment(self):
        ...
        for repo in Sources.mcu_environments[self.distro]:
            ...
            for package in repo.get_packages():
                if package.name in Sources.ignore_packages[
                    self.distro
                ] or package.name.endswith("_cpp"):
                    package.ignore()

                print(
                    "\t - Downloaded {}{}".format(
                        package.name, " (ignored)" if package.ignored else ""
                    )
                )
    ...
    def download_extra_packages(self):
        ...
        extra_repos = self.get_repositories_from_yaml(
            "{}/extra_packages.repos".format(self.packages_folder)
        )
        for repo_name in extra_repos:
            ...
            Repository(repo_name, repo_values["url"], self.distro, version).clone(
                self.mcu_src_folder
            )
        ...
        shutil.copytree(
            ...
            self.mcu_src_folder,
            ignore=shutil.ignore_patterns("extra_packages.repos"),
            dirs_exist_ok=True,
        )
    ...
    def build_mcu_environment(self, meta_file, toolchain_file, user_meta=""):
        ...
        colcon_command = ". {} && colcon build --merge-install --packages-ignore-regex=.*_cpp --metas {} {} {} --cmake-args -DCMAKE_POSITION_INDEPENDENT_CODE:BOOL=OFF  -DTHIRDPARTY=ON  -DBUILD_SHARED_LIBS=OFF  -DBUILD_TESTING=OFF  -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE={} -DPython3_EXECUTABLE=`which python`".format(
            self.python_env, common_meta_path, meta_file, user_meta, toolchain_file
        )
    ...
    def package_mcu_library(self):
        ...
        os.chdir(aux_folder)
        command = "{binutils}ar rc libmicroros.a $(ls *.o *.obj 2> /dev/null); rm *.o *.obj 2> /dev/null; {binutils}ranlib libmicroros.a".format(
            binutils=binutils_path
        )
        result = run_cmd(command)

        if 0 != result.returncode:
            print(
                "micro-ROS static library build failed: \n{}".format(
                    result.stderr.decode("utf-8")
                )
            )
            sys.exit(1)
    ...
    def resolve_binutils_path(self):
        if sys.platform == "darwin":
            ...
            print(
                "ERROR: GNU binutils not found. ({}) Please install binutils with homebrew: brew install binutils".format(
                    homebrew_binutils_path
                )
            )
            sys.exit(1)
